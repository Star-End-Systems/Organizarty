...
public class EditDecorationtGroupModel : PageModel
{
    private readonly ILogger<EditDecorationtGroupModel> _logger;
    private readonly SelectPartyUseCase _selectParty;
    private readonly EditPartyUseCase _editParty;

    public EditDecorationtGroupModel(ILogger<EditDecorationtGroupModel> logger, SelectPartyUseCase selectParty, EditPartyUseCase editParty)
    {
        _logger = logger;
    ...
    {
        var item = await _selectParty.FindDecoration(itemid);
    ...
    public async Task<IActionResult> OnPostAsync(Guid itemid)
    {
        try
        {
            var item = await _editParty.EditDecoration(new(itemid, Item.Quantity, Item.Note ?? ""));
        ...
        catch (Exception e)
        {
            ...
            ModelState.AddModelError(string.Empty, "Erro ao editar decoração, tente novamente mais tarde.");
            await OnGetAsync(itemid);
            return Page();
        }
    }
