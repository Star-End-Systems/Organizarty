...
public class ScheduleController : ControllerBase
{
    ...
    public record ScheduleResponse(Guid id, string Name, DateTime Date, String Type, decimal price, List<OrderResponse> orders)
    {
        ...
        => new ScheduleResponse(party.Id, party.Name, party.StartDate, party.Name, party.Price, new());

        public static ScheduleResponse FromParty(Schedule party, List<OrderResponse> orders)
        => new ScheduleResponse(party.Id, party.Name, party.StartDate, party.Name, party.Price, orders);
    }
    ...
    {
        var user = await _authHelper.GetUserFromToken(_authHelper.GetToken() ?? "") ?? throw new NotFoundException("User not found. Place use a valid token.");
